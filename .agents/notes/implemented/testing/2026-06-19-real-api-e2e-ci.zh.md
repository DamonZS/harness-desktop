...
一个与 ci.yml 分离的专用工作流 [.github/workflows/e2e.yml](../../../../.github/workflows/e2e.yml) 在配置 repo secret 时对外部 API 运行且仅运行 `pnpm run test:e2e`，仅在可信事件上触发。preflight 会记录 secret 是否存在；缺少凭证时输出明确通知并跳过真实 API 的构建与测试步骤。无密钥工作流保持独立，使可 fork 的质量门禁与消费 secret 的真实 API 门禁各自拥有不同的触发和凭证策略。
...
该门禁是一个*干净跳过的便利措施*，而非 secret 的安全边界（见 § 安全性——边界是 GitHub 自身在 `pull_request` 下对 fork 的 secret 扣留机制）。没有该门禁，fork 仍然无法读取密钥；只是会遇到令人困惑的无密钥跳过并浪费计算资源。

### Preflight：明确跳过无密钥运行

preflight 会将 `has_key=true|false` 写入 `$GITHUB_OUTPUT`。密钥为空时输出包含 `DEEPSEEK_API_KEY_EXTERNAL` 的 `::notice::`，并跳过构建与真实 API 测试步骤；配置密钥时运行完整套件。这样未配置可选外部凭证的仓库仍保持绿色，同时不会把跳过的测试伪装成已执行覆盖率；通知和步骤状态会保留在 Actions 日志中。
...
该设计带有已记录的约束表面：`pull_request` 触发器在密钥暴露方面的取舍（删除它可加强防护）、`if:` 门禁对基于作者的 Dependabot 检查的依赖、明确的无密钥跳过行为，以及对 `pull_request_target` 的严格禁止。上方公开仓库检查清单是操作配套——未来维护者在更改触发器集合或切换仓库可见性之前，应重新阅读本 Agent Note，而不是从头推导 fork/secret 模型。
