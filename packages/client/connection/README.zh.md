...
同一份已校验的 `trustedHosts` 会注入服务页面，并通过实例级 `options.trustedHosts` 传入 `installConnection(ctx, options)`。页面 authority 匹配其中一项时，浏览器会报告可用的 Host 特权面，设置改用 Host 持久化文档；未列入名单的远程 authority 仍保持内存模式。`options.location.host` 为精确 authority 条目保留端口；只含主机名的条目匹配任意端口。这些 UI 能力标记不替代 Host 身份认证或请求检查。
