...
The same validated `trustedHosts` authorities are injected into the served page and passed to `installConnection(ctx, options)` as instance-local `options.trustedHosts`. When the page authority matches one of them, the browser reports the privileged Host surface and settings use the persistent Host document; an unlisted remote authority remains memory-only. `options.location.host` preserves the port for exact-authority entries; hostname-only entries match any port. These UI capability flags do not replace Host authentication or request checks.
