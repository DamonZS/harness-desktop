...
The same validated `trustedHosts` authorities are injected into the served page. When the page authority matches one of them, the browser reports the privileged Host surface and settings use the persistent Host document; an unlisted remote authority remains memory-only.
