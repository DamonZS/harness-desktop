...
describe('connection client apply', () => {
  ...
  it.each([
    ['harness.example:8443', ['harness.example:8443'], true],
    ['harness.example:9443', ['harness.example:8443'], false],
    ['other.example:8443', ['harness.example:8443'], false],
    ['harness.example:8443', ['harness.example'], true],
    ['', ['harness.example'], false],
  ])('preserves deployment settings access for %s without widening the trusted authority', async (host, trustedHosts, allowed) => {
    vi.stubGlobal('location', { hostname: host.split(':')[0], host })
    vi.stubGlobal('__DSH_CONNECTION_TRUSTED_HOSTS__', trustedHosts)
    expect((await mount()).isLoopback).toBe(allowed)
  })
