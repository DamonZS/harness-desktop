...
describe('spawn construction (pure, every platform)', () => {
  ...
  class CapturingSubprocessRuntime extends SubprocessRuntime {
    ...
    exited: Promise<boolean> = Promise.resolve(true)
    exitChecks = 0
    stderrText = ''
    ...
    override spawn(spec: SubprocessSpawnSpec): SubprocessHandle {
      ...
      return {
        ...
        terminate: () => {},
        waitForExit: () => { this.exitChecks++; return this.exited },
      }
    }
  }
  ...
  it.each(['timeout', 'abort'] as const)('reports %s during managed startup after process cleanup', async (kind) => {
    const ctx = createContext()
    const subprocess = new CapturingSubprocessRuntime(ctx)
    await ctx.plugin(PwshLocalExecutor)
    const outcome = Promise.withResolvers<SubprocessOutcome>()
    const exited = Promise.withResolvers<boolean>()
    subprocess.done = outcome.promise
    subprocess.exited = exited.promise
    const controller = new AbortController()
    let settled = false
    const pending = ctx.shell.run(ctx.shell.resolve({
      command: 'Write-Output unused', signal: controller.signal, timeoutMs: kind === 'timeout' ? 10 : 10000,
    })).then((result) => { settled = true; return result })
    const signal = subprocess.specs[0]!.signal!
    signal.addEventListener('abort', () => { outcome.reject(signal.reason) }, { once: true })
    if (kind === 'abort') controller.abort()
    try {
      await expect.poll(() => subprocess.exitChecks).toBe(1)
      expect(settled).toBe(false)
    } finally { exited.resolve(true) }
    await expect(pending).resolves.toMatchObject({
      exitCode: null, signal: null, timedOut: kind === 'timeout', aborted: kind === 'abort',
    })
  })

  it.each([false, true])('preserves unrelated provider failures when aborted=%s', async (aborted) => {
    const ctx = createContext()
    const subprocess = new CapturingSubprocessRuntime(ctx)
    await ctx.plugin(PwshLocalExecutor)
    const outcome = Promise.withResolvers<SubprocessOutcome>()
    subprocess.done = outcome.promise
    const controller = new AbortController()
    const pending = ctx.shell.run(ctx.shell.resolve({ command: 'Write-Output unused', signal: controller.signal }))
    if (aborted) controller.abort()
    const failure = new Error('provider failure')
    outcome.reject(failure)
    await expect(pending).rejects.toBe(failure)
    expect(subprocess.exitChecks).toBe(0)
  })
